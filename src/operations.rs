...
pub fn floor_divide(a: Value, b: Value) -> Result<Value, InterpreterError> {
    match (a, b) {
        ...
        (a, b) => Err(InterpreterError::BinaryTypeError(BinaryOp::FloorDiv, a.get_type(), b.get_type())),
    }
}
...
pub fn less_than(a: Value, b: Value) -> Result<Value, InterpreterError> {
    match (a, b) {
        ...
        (a, b) => Err(InterpreterError::BinaryTypeError(BinaryOp::LessThan, a.get_type(), b.get_type())),
    }
}
...
pub fn less_than_or_equal(a: Value, b: Value) -> Result<Value, InterpreterError> {
    match (a, b) {
        (Value::Number(a), Value::Number(b)) => Ok(Value::Bool(a <= b)),
        (a, b) => Err(InterpreterError::BinaryTypeError(BinaryOp::LessThanOrEqual, a.get_type(), b.get_type())),
    }
}
pub fn greater_than(a: Value, b: Value) -> Result<Value, InterpreterError> {
    match (a, b) {
        (Value::Number(a), Value::Number(b)) => Ok(Value::Bool(a > b)),
        (a, b) => Err(InterpreterError::BinaryTypeError(BinaryOp::GreaterThan, a.get_type(), b.get_type())),
    }
}
...
pub fn greater_than_or_equal(a: Value, b: Value) -> Result<Value, InterpreterError> {
    match (a, b) {
        (Value::Number(a), Value::Number(b)) => Ok(Value::Bool(a >= b)),
        (a, b) => Err(InterpreterError::BinaryTypeError(BinaryOp::GreaterThanOrEqual, a.get_type(), b.get_type())),
    }
}
